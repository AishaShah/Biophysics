...
args['input_structure_path'] = '/home/aisha/BIOPHYSICS/Project/6M0J.cif'
args['output_structure_path'] = '/home/aisha/BIOPHYSICS/Project/6m0jnn_fixed.pdb'
args['output_structure_path_charges'] = '/home/aisha/BIOPHYSICS/Project/6m0jnn_fixed.pdbqt'
args['debug'] = False
args['verbose'] = False
...
st_c.altloc()  # when some residue have more than one resolution in crystal structure
...
st_c.altloc('occupancy')  # select by ocuppanct
...
st_c.ligands('All')  # removing ligands
...
st_c.amide()  # they are glutamins and asparagins
...
st_c.backbone()  # two items of oxy were missing
...
# all organic residues may have more than one version if we add H, if we say do this at
st_c.add_hydrogen('auto')
# pH 7 then it makes correct correction the neutral ones will remain neutral acidic will remain acidic
...
# st_c.add_hydrogen('--add_charges --add_mode auto') ## its giving some updating part of charges charge of -28.16 charge but there are alot of atom so -28 is ok but 0.65 is problem it should be integer, it is because of histidines maybe


# In[34]:


os.system('check_structure -i ' + args['output_structure_path'] + ' -o ' +
          args['output_structure_path_charges'] + ' add_hydrogen --add_charges --add_mode auto')


# In[35]:


st_c._save_structure('/home/aisha/BIOPHYSICS/Project/6m0jnn.pdbqt')
